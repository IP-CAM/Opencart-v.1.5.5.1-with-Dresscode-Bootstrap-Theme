...
class ControllerCatalogProduct extends Controller {
    ...
    protected function dbCheck(){
  	...
  	protected function getForm() {
		...
		if (isset($this->request->post['make'])) {
      		$this->data['make'] = $this->request->post['make'];
    	} elseif (!empty($product_info)) {
			$this->data['make'] = $product_info['make'];
		} else {
      		$this->data['make'] = '';
    	}
		$this->data['gModel'] = '';
		$this->data['sModel'] = '';
		if(isset($product_info['make'])){
			$isModel = $this->model_catalog_product->getAllModelByMakeId($product_info['make']);
			$str = '';
			$str2 = '';
			/*foreach($isModel as $ggModel){
				if($this->data['model'] !='' && $this->data['model'] == $ggModel['modelId']){
					$str = $str . "<option selected='selected' value='$ggModel[modelId]'>$ggModel[ModelName]</option>";
				}
				else{
					$str = $str . "<option value='$ggModel[modelId]'>$ggModel[ModelName]</option>";
				}
			}*/
			foreach($isModel as $ggModel){
				if(isset($this->data['model'])){
					$svar = explode(",",$this->data['model']);
					foreach($svar as $sss){
						if($sss == $ggModel['modelId']){
							if($str2 !=''){
								$str2 = $str2.','.$ggModel['ModelName'];	
							}
							else{
								$str2 = $ggModel['ModelName'];
							}
						}
					}
				}
			...
			$this->data['sModel'] = $str2;
			$this->data['gModel'] = $str;
		}
		//
	...
	public function getAllMake(){
		$this->load->model('catalog/product');
		$make = $this->model_catalog_product->getAllMakeList();
		foreach($make as $makes){
			$this->data['make_data'][] = array(
				'makeId'	=>	$makes['makeId'],
