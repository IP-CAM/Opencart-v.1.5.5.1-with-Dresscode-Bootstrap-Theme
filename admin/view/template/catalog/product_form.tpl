...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="content">
      <div id="tabs" class="htabs"><a href="#tab-general"><?php echo $tab_general; ?></a><a href="#tab-data"><?php echo $tab_data; ?></a><a href="#tab-links"><?php echo $tab_links; ?></a><a href="#tab-attribute"><?php echo $tab_attribute; ?></a><a href="#tab-option"><?php echo $tab_option; ?></a><a href="#tab-discount"><?php echo $tab_discount; ?></a><a href="#tab-special"><?php echo $tab_special; ?></a><a href="#tab-image"><?php echo $tab_image; ?></a><a href="#tab-reward"><?php echo $tab_reward; ?></a><a href="#tab-design"><?php echo $tab_design; ?></a><a href="#tab-youtube"><?php echo $tab_youtube; ?></a></div>
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        ...
        <div id="tab-data">
          <table class="form">
             <tr>
              <td><span class="required">*</span><?php echo $text_make; ?></td>
              <td><select style="width:135px;" onchange="getModel(this);" id="make" name="make"><option value="-1">--Select Make--</option><?php foreach($make_data as $makes){if(isset($make) && $make==$makes['makeId']){echo "<option value='$makes[makeId]' selected='true'>$makes[MakeName]</option>";}else{echo "<option value='$makes[makeId]'>$makes[MakeName]</option>";}}?></select></td>
            </tr>
			<tr>
              <td><span class="required">*</span><?php echo $entry_model; ?></td>
              <td><span id="ddlModel"><select style="width:135px;" onchange="loadModel(this);" id="model" name="model"><option value="-1">--Select Model--</option><?php if(isset($gModel)){echo $gModel;}else{echo "";}?></select></span>&nbsp;&nbsp;<span id='ajaxloader' style="display:none;"><img src="view/image/al.gif"></span><span><input type="button" onclick="mclear();" value="clear" /></span><span id='zmodel' style="margin-left:20px;"><?php if(isset($sModel)){echo $sModel;}else{echo "";}?></span><input id="hfsModel" type="hidden" name="hfsModel" value="<?php if(isset($model)){echo $model;}else{echo "";}?>" /></td>
            </tr>
			<tr>
              <td><span class="required">*</span><?php echo $text_year; ?></td>
              <td><select id="year" style="width:135px;" onchange="loadYear(this);" name="year"><option value="-1">--Select Year--</option><option value="ALL">ALL</option><?php $currYear = date("Y");for($i=$currYear;$i>=1967;$i--){echo "<option value='$i'>$i</option>";}?></select>&nbsp;&nbsp;<span><input type="button" onclick="sclear();" value="clear" /></span><span id='zyear' style="margin-left:20px;"><?php if(isset($year)){echo $year;}else{echo "";}?></span><input id="hfsYear" type="hidden" name="hfsYear" value="<?php if(isset($year)){echo $year;}else{echo "";}?>" /></td>
            </tr>
			<!--<tr>
              <td><span class="required">*</span> <?php echo $entry_model; ?></td>
...
<script type="text/javascript">
	function getModel(obj){
		if(obj.value > 0){
			...
			$.ajax({
        	type: "GET",
			url: "utility/getModel.php?mId=" + obj.value,
	...
	function loadYear(obj){
		if($('#zyear').html() != ''){
			$('#zyear').html($('#zyear').html()+','+obj.value);
			$('#hfsYear').val($('#hfsYear').val()+','+obj.value);
		}
		else{
			$('#zyear').html(obj.value);
			$('#hfsYear').val(obj.value);
		}
	}
	function sclear(){
		$('#hfsYear').val('');
	...
	function loadModel(obj){
		if($('#zmodel').html() != ''){
			$('#zmodel').html($('#zmodel').html()+','+$("#model option:selected").text());
			$('#hfsModel').val($('#hfsModel').val()+','+obj.value);
		}
		else{
			$('#zmodel').html($("#model option:selected").text());
			$('#hfsModel').val(obj.value);
		}
	}
	function mclear(){
		$('#hfsModel').val('');
